...
class GymnaxLoop:
    ...
    * training=True, agent.num_off_policy_updates_per_cycle() > 0: the specified number
    ...
    def __init__(
        ...
        Args:
            ...
            devices: sequence of devices for data parallelism.
                ...
                If not set, runs on jax.local_devices().
        """
        self._env = env
    ...
    def reset_env(self) -> tuple[EnvState, EnvStep]:
        ...
        obs, env_state = jax.pmap(jax.vmap(self._env_reset), axis_name=self._PMAP_AXIS_NAME, devices=self._devices)(
            env_keys
        )
    ...
    def run(
        ...
        for cycle_num in cycles_iter:
            ...
            cycle_result = self._run_cycle_and_update(agent_state, env_state, env_step, key, steps_per_cycle)

            cycle_result_device_0 = jax.tree.map(lambda x: x[0] if isinstance(x, jax.Array) else x, cycle_result)
    ...
    def _run_cycle(
        ...
        def scan_body(inp: StepCarry[_StepState], _) -> tuple[StepCarry[_StepState], tuple[EnvStep, dict[Any, Any]]]:
