...
def test_run_experiment_no_eval_cycles(env_backend: str, num_eval_cycles: int):
  ...
  if not num_eval_cycles:
    ...
    steps_between_evals = (
      experiment.num_train_cycles // experiment.num_eval_cycles * experiment.steps_per_cycle
    )
    step_nums = sorted(eval_metrics.keys())
    assert all(
      step_nums[i] == (i + 1) * steps_between_evals + experiment.steps_per_cycle
...
def test_checkpointing(env_backend: str, tmp_path):
  ...
  _, _, restored_loop_state = _restore_checkpoint(
    checkpoint_manager,
    CheckpointRestoreMode.BEST,
    experiment._agent_obj,
    experiment.env,
    loop_state,
  )
