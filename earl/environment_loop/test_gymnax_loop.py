...
class TestGymnaxLoop(parameterized.TestCase):
    ...
    def test_gymnax_loop(self, inference: bool, num_off_policy_updates: int):
        ...
        loop = GymnaxLoop(
            env, env_params, agent, num_envs, next(key_gen), metric_writer=metric_writer, inference=inference
        )
        num_cycles = 2
        steps_per_cycle = 10
        ...
        self.assertGreater(env.num_actions, 0)
    ...
    def test_run_with_state(self):
        ...
        self.assertIsInstance(prev_action, jax.Array)
        state = State(
            agent_state,
            env_state,
            EnvStep(
        ...
        self.assertEqual(env.reset.call_count, 0)
    ...
    def test_observe_cycle(self):
        ...
        def observe_cycle(cycle_result: CycleResult) -> Metrics:
            ...
            self.assertIn("discount", cycle_result.step_infos)
            return {"ran": True}
